...
const handleImageUploads = async (files: UniversityFiles, existingImages?: any) => {
    ...
    if (files?.logo?.[0]) {
        ...
        const logoResult = await uploadToCloudinary(files.logo[0], 'universities/logos') as { public_id: string; secure_url: string };
    ...
    if (files?.coverImage?.[0]) {
        ...
        const coverResult = await uploadToCloudinary(files.coverImage[0], 'universities/covers') as { public_id: string; secure_url: string };
...
export const uploadUniversityImages = (req: Request, res: Response, next: NextFunction) => {

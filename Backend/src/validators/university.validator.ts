...
export const getUniversitiesSchema = z.object({
    page: z.string().optional().transform(val => parseInt(val || '1') || 1).pipe(z.number().int().positive()),
    limit: z.string().optional().transform(val => parseInt(val || '10') || 10).pipe(z.number().int().min(1).max(100)),
